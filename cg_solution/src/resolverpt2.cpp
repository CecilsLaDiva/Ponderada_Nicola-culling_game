...
const Point TARGET_POS = {14, 14};

class MazeResolverPt2 : public rclcpp::Node {
public:
    MazeResolverPt2() : Node("maze_resolver_pt2_cpp") {
        ...
        sensor_sub_ = this->create_subscription<cg_interfaces::msg::RobotSensors>(
            "/culling_games/robot_sensors", 10, std::bind(&MazeResolverPt2::sensor_callback, this, std::placeholders::_1));
    ...
    void run() {
        ...
        RCLCPP_INFO(this->get_logger(), "Iniciando Exploração Guiada (C++)...");
        reset_game();

        // Espera sensor inicial
        wait_for_sensor();
        ...
        while (!stack_.empty()) {
            ...
            // Check vizinhos para alvo imediato
            ...
            // Heurística
            struct MoveOption {
                int dist; std::string move; int dr, dc, nr, nc;
            };
            std::vector<MoveOption> valid_moves;
            ...
            for (auto [move, dr, dc] : dirs) {
                ...
                if (is_valid(nr, nc)) {
                    ...
                    if ((val == "f" || val == "t") && visited_.find({nr, nc}) == visited_.end()) {
                        int dist = std::abs(nr - TARGET_POS.r) + std::abs(nc - TARGET_POS.c);
                        valid_moves.push_back({dist, move, dr, dc, nr, nc});
                    }
                }
            }

            std::sort(valid_moves.begin(), valid_moves.end(), [](const MoveOption& a, const MoveOption& b){
                return a.dist < b.dist;
            });

            if (!valid_moves.empty()) {
                auto best = valid_moves[0];
                if (move_robot(best.move)) {
                    visited_.insert({best.nr, best.nc});
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<MazeResolverPt2>();
    node->run();
    rclcpp::shutdown();
    return 0;
}